...
dashboard_current_listen() {
    local line value
    line="$(dashboard_env_line MAVLINK_ANYWHERE_DASHBOARD_LISTEN)"
    if [[ -n "$line" ]]; then
        value="${line#*=}"
        value="${value#\'}"
        value="${value%\'}"
    ...
    if [[ -f "${DASHBOARD_SERVICE_FILE:-/etc/systemd/system/${DASHBOARD_SERVICE}.service}" ]]; then
        value="$(sed -n 's/.*--listen[ =]\([^ ]*\).*/\1/p' "${DASHBOARD_SERVICE_FILE:-/etc/systemd/system/${DASHBOARD_SERVICE}.service}" | head -1)"
        [[ -n "$value" ]] && printf '%s\n' "$value" && return 0
    fi
    printf '127.0.0.1:%s\n' "$DASHBOARD_PORT"
}
...
write_dashboard_env_file() {
    ...
    local open_lab="$6"
    local listen_addr="${7:-127.0.0.1:${DASHBOARD_PORT}}"
    local tmp_file line
...

    {
        ...
        printf 'MAVLINK_ANYWHERE_DASHBOARD_LISTEN='
        env_quote "$listen_addr"
        printf '\n'
...
configure_dashboard_auth() {
    ...
    if [[ "${DASHBOARD_OPEN_LAB_MODE:-false}" == "true" ]]; then
        write_dashboard_env_file "" "" false "" false true "$listen_addr"
    ...
    if [[ "${DASHBOARD_DISABLE_AUTH:-false}" == "true" ]]; then
        write_dashboard_env_file "" "" false "$api_token" "$preserve_api_token" false "$listen_addr"
    ...
    write_dashboard_env_file "$auth_user" "$auth_hash" "$preserve_auth" "$api_token" "$preserve_api_token" false "$listen_addr"
...
build_dashboard_binary_from_source() {
    ...
    version="v${MAVLINK_ANYWHERE_VERSION}"
...
ExecStart=${DASHBOARD_INSTALL_DIR}/${DASHBOARD_BINARY_NAME}
Restart=on-failure
RestartSec=5
MemoryMax=30M
MemoryHigh=20M
...
install_dashboard() {
    ...
    if dashboard_is_local_only "$listen_addr"; then
        ...
        ma_log_info "Network access when needed: sudo mla dashboard expose"
    else
        ma_log_success "Dashboard listening on: http://${listen_addr}"
        ma_log_info "Return to local-only anytime: sudo mla dashboard hide"
    fi
    return 0
}
